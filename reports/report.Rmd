...
```{r}
df <- as.data.frame.table(table(baseline$PI_MUTATION_COUNT))
names(df) <- c("Number","Count")
...
```{r nrtiresistance}
df <- as.data.frame.table(table(baseline$NRTI_MUTATION_COUNT))
names(df) <- c("Number","Count")
p <- ggplot(df,aes(x="",y=Count,fill=Number))+geom_bar(width=1,stat="identity")+coord_polar("y", start=0)+blank_theme + theme(axis.text.x=element_blank())
p
```
...
```{r nnrtipie}
df <- as.data.frame.table(table(baseline$NNRTI_MUTATION_COUNT))
names(df) <- c("Number","Count")
p <- ggplot(df,aes(x="",y=Count,fill=Number))+geom_bar(width=1,stat="identity")+coord_polar("y", start=0)+blank_theme + theme(axis.text.x=element_blank())
p
```
